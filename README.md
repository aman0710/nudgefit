...
```
nudgefit/
├── src/main/java/com/nudgefit/
│   ├── NudgeFitApplication.java          # Entry point, IST timezone init
│   ├── ai/
│   │   ├── GeminiAiService.java          # Gemini REST client with rate limiting & retry
│   │   └── PromptBuilder.java            # Template-based prompt engine
│   ├── config/
│   │   ├── GeminiConfig.java             # Gemini API key & model config
│   │   ├── RedisConfig.java              # StringRedisTemplate bean
│   │   └── TwilioConfig.java             # Twilio credentials & init
│   ├── controller/
│   │   ├── WhatsAppWebhookController.java # Twilio webhook endpoint
│   │   └── HealthController.java          # /health uptime endpoint
│   ├── model/
│   │   ├── entity/                        # JPA entities (User, DailyLog, FoodEntry, WorkoutEntry)
│   │   ├── dto/                           # Records for Gemini responses & events
│   │   └── enums/                         # 9 enums (ConversationState, Intent, MealType, etc.)
│   ├── repository/                        # Spring Data JPA repositories
│   ├── scheduler/                         # 3 cron jobs (morning, evening, inactivity)
│   ├── service/                           # Business logic (8 service classes)
│   └── util/
│       └── MacroCalculator.java           # BMR/TDEE/macro/timeline calculations
├── src/main/resources/
